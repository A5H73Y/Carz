...
public class ValidationUtils {
	...
	/**
	 * Validate that the vehicle is a valid Vehiclez vehicle.
	 * Checks to see if the vehicle is a Minecart that isn't on rails.
	 *
	 * @param vehicle vehicle
	 * @return is Vehiclez vehicle
	 */
	public static boolean isAVehiclezVehicle(Vehicle vehicle) {
		...
		CarDataPersistence persistence = Vehiclez.getInstance().getCarDataPersistence();

		if (!persistence.has(VehicleDetailKey.VEHICLE_TYPE, vehicle)) {
			return false;
		}

		return Vehiclez.getInstance().getCarController().doesCarTypeExist(
	...
	public static boolean canPurchaseCar(Player player, String carType, double costOverride) {
		...
		if (player.getInventory().contains(Material.MINECART)
				&& Vehiclez.getInstance().getConfig().getBoolean("PreventCarPurchaseWhenExisting")) {
			TranslationUtils.sendTranslation("Error.HaveCar", player);
			return false;
		}

		if (!Vehiclez.getInstance().getCarController().doesCarTypeExist(carType.toLowerCase())) {
			TranslationUtils.sendTranslation("Error.UnknownCarType", player);
			return false;
		}

		double cost = costOverride > 0 ? costOverride :
				Vehiclez.getDefaultConfig().getDouble("CarTypes." + carType.toLowerCase() + ".Cost");
		return Vehiclez.getInstance().getEconomyApi().canPurchase(player, cost);
	}
	...
	public static boolean canPurchaseUpgrade(Player player, double costOverride) {
		...
		if (!isAVehiclezVehicle((Vehicle) player.getVehicle())) {
			return false;
		}
		...
		Car currentCar = Vehiclez.getInstance().getCarController().getCar((Minecart) player.getVehicle());

		if (currentCar.getMaxSpeed() + Vehiclez.getDefaultConfig().getUpgradeIncrement()
		...
		double cost = costOverride > 0 ? costOverride :
				Vehiclez.getDefaultConfig().getDouble("Vault.Cost.Upgrade");
		return Vehiclez.getInstance().getEconomyApi().canPurchase(player, cost);
	}
	...
	public static boolean canPurchaseFuel(Player player, double costOverride) {
		...
		if (!Vehiclez.getInstance().getCarDataPersistence().has(VehicleDetailKey.VEHICLE_FUEL, player.getVehicle())) {
		...
		if (costOverride > 0) {
			...
			double remainingFuel = Double.parseDouble(Vehiclez.getInstance().getCarDataPersistence().getValue(
					VehicleDetailKey.VEHICLE_FUEL, player.getVehicle()));
			cost = Vehiclez.getInstance().getEconomyApi().getRefuelCost(remainingFuel);
		}
		return Vehiclez.getInstance().getEconomyApi().canPurchase(player, cost);
	}
	...
	public static boolean canClaimCar(Player player) {
		...
		if (Vehiclez.getInstance().getCarDataPersistence().has(VehicleDetailKey.VEHICLE_OWNER, player.getVehicle())) {
			String owner = Vehiclez.getInstance().getCarDataPersistence()
	...
	public static boolean canRemoveCarOwnership(Player player) {
		...
		String owner = Vehiclez.getInstance().getCarDataPersistence().getValue(
	...
	public static boolean canPlaceCar(Player player, Material placedMaterial) {
		...
		BlocksConfig blocksConfig = (BlocksConfig) Vehiclez.getConfig(ConfigType.BLOCKS);
		...
		int maxOwnedCars = Vehiclez.getDefaultConfig().getMaxPlayerOwnedCars();
		if (maxOwnedCars > 0
